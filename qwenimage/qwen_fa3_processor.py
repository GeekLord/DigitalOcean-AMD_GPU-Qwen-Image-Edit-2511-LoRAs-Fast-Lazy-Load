...
# ---------------------------------------------------------------------------
# FA3 / ROCm availability check
# ---------------------------------------------------------------------------

def _is_rocm() -> bool:
    """Return True when PyTorch is compiled for AMD ROCm (HIP)."""
    return getattr(torch.version, "hip", None) is not None


def _is_blackwell() -> bool:
    """Return True when the current default CUDA device is an sm_100 (Blackwell) GPU."""
    if not torch.cuda.is_available() or _is_rocm():
        return False
...
if _is_rocm():
    _fa3_unavailable_reason = (
        "AMD ROCm GPU detected (e.g., AMD Instinct MI300X). "
        "FlashAttention-3 CUDA kernel is bypassed in favor of PyTorch SDPA, "
        "which leverages AMD ROCm-optimized FlashAttention / Composable Kernel backends."
    )
elif _is_blackwell():
    _fa3_unavailable_reason = (
